...
class des():
    ...
    def substitute(self, d_e):#Substitute bytes using SBOX
        ...
        for i, block in enumerate(subblocks):
            s_box_output = self.compute_s_box(block, i)

            s_box_output_list = [int(bit) for bit in s_box_output]
            result.extend(s_box_output_list)
        return result
    ...
    def compute_s_box(self,block,round):
        ...
        s_box = S_BOX[round % len(S_BOX)]
        # Compute the row and column index
        row = int("".join([str(x) for x in [block[0], block[5]]]), 2)
